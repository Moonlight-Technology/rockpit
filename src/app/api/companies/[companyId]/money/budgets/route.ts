...
import {
  getOrCreateCompanyMoneyBudget,
  upsertCompanyMoneyBudget,
} from "@/lib/company-money-service";
import { upsertCompanyMoneyBudgetSchema } from "@/lib/validators/company-money";
...
export async function GET(
  ...
  const result = await getOrCreateCompanyMoneyBudget({ userId, companyId, month });
...
export async function POST(
  ...
  const parsed = upsertCompanyMoneyBudgetSchema.safeParse(payload);
  if (!parsed.success) return validationError("Invalid budget payload.");

  const { companyId } = await params;
  const result = await upsertCompanyMoneyBudget({ userId, companyId, payload: parsed.data });
  if (!("data" in result)) {
