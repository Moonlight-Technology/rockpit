...
import { createCompanyReceivablePaymentSchema } from "@/lib/validators/company-money";
...
export async function GET(
  ...
  const result = await listCompanyReceivables({ userId, companyId });
...
export async function POST(
  ...
  const parsed = createCompanyReceivablePaymentSchema.safeParse(payload);
  if (!parsed.success) return validationError("Invalid receivable payment payload.");

  const { companyId } = await params;
  const result = await recordCompanyReceivablePayment({
    userId,
    companyId,
    payload: parsed.data,
  });
