...
const companyMoneyApiBasePath = (companyId: string) => `/api/companies/${companyId}/money`;
...
export function CompanyExpenseManager({ companyId, companyName }: CompanyExpenseManagerProps) {
  ...
  const loadMoneyData = async () => {
    ...
    try {
      const [nextAccounts, nextCategories, nextTransactions, nextBudget, nextWishlist, nextReceivables] =
        await Promise.all([
          readApi<MoneyAccount[]>(`${companyMoneyApiBasePath(companyId)}/accounts`),
          readApi<MoneyCategory[]>(`${companyMoneyApiBasePath(companyId)}/categories`),
          readApi<MoneyTransaction[]>(
            `${companyMoneyApiBasePath(companyId)}/transactions?month=${month}`
          ),
          readApi<MoneyBudget>(`${companyMoneyApiBasePath(companyId)}/budgets?month=${month}`),
          readApi<WishlistItem[]>(`${companyMoneyApiBasePath(companyId)}/wishlist`),
          readApi<Receivable[]>(`${companyMoneyApiBasePath(companyId)}/receivables`),
        ]);
      ...
      if (!nextAccounts || !nextCategories || !nextTransactions || !nextBudget || !nextWishlist || !nextReceivables) {
        setError("Sebagian data Expense Manager belum tersedia. Coba muat ulang.");
      }
    } finally {
      setLoading(false);
    }
  ...
  const submitTransaction = async (event: FormEvent) => {
    ...
    setSaving(true);
    try {
      await postApi(`${companyMoneyApiBasePath(companyId)}/transactions`, payload);
      setActiveForm(null);
  ...
  const submitTransactionDetail = async (event: FormEvent) => {
    ...
    try {
      await patchApi(
        `${companyMoneyApiBasePath(companyId)}/transactions/${selectedTransaction.id}`,
        payload
      );
  ...
  const deleteSelectedTransaction = async () => {
    ...
    try {
      await deleteApi(`${companyMoneyApiBasePath(companyId)}/transactions/${selectedTransaction.id}`);
      setSelectedTransaction(null);
      await loadMoneyData();
    } catch (err) {
  ...
  const submitAccount = async (event: FormEvent) => {
    ...
    try {
      await postApi(`${companyMoneyApiBasePath(companyId)}/accounts`, {
  ...
  const submitBudget = async (event: FormEvent) => {
    ...
    try {
      await postApi(`${companyMoneyApiBasePath(companyId)}/budgets`, {
        month,
  ...
  const submitCategory = async (event: FormEvent) => {
    ...
    try {
      ...
      if (categoryForm.id) {
        await patchApi(`${companyMoneyApiBasePath(companyId)}/categories`, payload);
      } else {
        await postApi(`${companyMoneyApiBasePath(companyId)}/categories`, payload);
      }
  ...
  const submitWishlist = async (event: FormEvent) => {
    ...
    try {
      await postApi(`${companyMoneyApiBasePath(companyId)}/wishlist`, {
  ...
  const toggleWishlistItem = async (item: WishlistItem, checked: boolean) => {
    ...
    try {
      await patchApi(`${companyMoneyApiBasePath(companyId)}/wishlist`, {
        id: item.id,
  ...
  const submitRepayment = async (receivable: Receivable) => {
    ...
    try {
      await postApi(`${companyMoneyApiBasePath(companyId)}/receivables`, {
    ...
    <div className="min-h-screen bg-[linear-gradient(180deg,#f9fafc_0%,#f3f5fa_48%,#eef2f9_100%)]">
      <main className="mx-auto flex w-full max-w-7xl flex-col gap-4 px-4 py-5 md:px-8 md:py-8">
        <header className="flex flex-col gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold tracking-tight md:text-3xl">Expense Manager</h1>
